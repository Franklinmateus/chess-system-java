...
public class ChessMatch {
	...
	public boolean[][] possibleMoves(ChessPosition sourcePosition){
		Position position = sourcePosition.toPositio();
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		Position source = sourcePosition.toPositio();
		Position target = targetPosition.toPositio();
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		Position pos = promoted.getChessPosition().toPositio();
		...
		//anular falsos checks
		check = testCheck(currentPlayer) ? true : false;
		
		//se der check na hora de promover
	...
	private void validateTargetPosition (Position source, Position target) {
		if(!board.piece(source).possibleMove(target)) {
			throw new ChessException("A peca escolhida não pode se mover para posição escolhida.");
		}
	}
	...
	private boolean testCheck(Color color) {
		Position kingPosition = king(color).getChessPosition().toPositio();
	...
	private boolean testCheckMate(Color color) {
		...
		for(Piece p : list) {
			...
			for(int i=0; i<board.getRows(); i++) {
				for(int j=0; j<board.getColumns(); j++) {
					if(mat[i][j]) {
						Position source = ((ChessPiece)p).getChessPosition().toPositio();
	...
	private void placeNewPiece(char column, int row, ChessPiece piece) {
		board.placePiece(piece, new ChessPosition(column, row).toPositio());
